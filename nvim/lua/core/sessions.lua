...
vim.opt.sessionoptions = "curdir,folds,tabpages,winsize,winpos,terminal,localoptions"
...
if vim.fn.argc() == 0 then
    ...
    if vim.fn.filereadable(session_file) == 1 then
        ...
        vim.g.session_restore_pending = true
    end
end

vim.api.nvim_create_autocmd("VimEnter", {
    callback = function()
        if not startup_session_file then
            return
        end

        vim.cmd("silent! set winminwidth=1 winwidth=1 winminheight=1 winheight=1")
        local ok, err = pcall(vim.cmd, "source " .. vim.fn.fnameescape(startup_session_file))
        vim.g.session_restore_pending = false

        if not ok then
            vim.notify("Failed to restore session: " .. err, vim.log.levels.ERROR)
        end
    end,
})
